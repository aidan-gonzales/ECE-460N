...
//I added the signals below
//
//MEM stage signals needed in previous stages
int// mem_pcmux, // fix me
    mem_drid,
    v_mem_ld_cc,
    v_mem_ld_reg,
    target_pc,
...
//AGEX stage signals needed in previous stages
int agex_drid,
    v_agex_ld_cc,
    v_agex_ld_reg;

//There are no signals in the DE or FE stages that are needed in previous stages (other than the stalls in DE)
...
void MEM_stage() {
	...
	int we0 = (Get_DCACHE_RW(PS.MEM_CS) && (Get_DATA_SIZE(PS.MEM_CS) || !(PS.MEM_ADDRESS & 0x0001)));

	int we1 = (Get_DCACHE_RW(PS.MEM_CS) && (Get_DATA_SIZE(PS.MEM_CS) || (PS.MEM_ADDRESS & 0x0001)));

	switch (Get_DATA_SIZE(PS.MEM_CS)) {
		case 0:
			switch (PS.MEM_ADDRESS & 0x0001) {
				case 0:
					dcache_in = (PS.MEM_ALU_RESULT & 0x00FF);
					break;
				case 1:
					dcache_in = ((PS.MEM_ALU_RESULT & 0x00FF) << 8);
					break;
			}
			break;
		case 1:
			dcache_in = PS.MEM_ALU_RESULT;
			break;
	}

	dcache_access(PS.MEM_ADDRESS, &dcache_out, dcache_in, &dcache_r, we0, we1);
	...
	if (v_dcache_en) {
		switch (Get_DATA_SIZE(PS.MEM_CS)) {
			case 0:
				switch (PS.MEM_ADDRESS & 0x0001) {
					case 0:
						data = (dcache_out & 0x00FF);
						if (data & 0x0080) {
							data |= 0xFF00;
						}
						break;
					case 1:
						data = ((dcache_out & 0xFF00) >> 8);
						if (data & 0x0080) {
							data |= 0xFF00;
						}
						break;
	...
	//signals to send to previous stages
	...
	//br logic and other signals logic
	if (PS.MEM_V) {
		int tempN = PS.MEM_IR & 0x0800;
		int tempZ = PS.MEM_IR & 0x0400;
		int tempP = PS.MEM_IR & 0x0200;
		int otherN = PS.MEM_CC & 0x0004;
		int otherZ = PS.MEM_CC & 0x0002;
		int otherP = PS.MEM_CC & 0x0001;
		if (Get_UNCOND_OP(PS.MEM_CS) || (Get_BR_OP(PS.MEM_CS) && ((tempN && otherN) || (tempZ && otherZ) || (tempP && otherP)))) {
			mem_pcmux = 1;
	...
	} else { // wasn't sure about this else statement at first, but I think it's needed
		mem_pcmux = 0;
	}
...
void AGEX_stage() {
	...
	//logic to update signals needed by previous stages
	...
	if (LD_MEM) {
		...
		switch (Get_ADDR2MUX(PS.AGEX_CS)) {
			case 0:
				...
				break;
			case 1:
				temp2 = PS.AGEX_IR & 0x003F;
				if (temp2 & 0x0020) {
					temp2 |= 0xFFC0;
				}
				break;
			case 2:
				temp2 = PS.AGEX_IR & 0x01FF;
				if (temp2 & 0x0100) {
					temp2 |= 0xFE00;
				}
				break;
			case 3:
				temp2 = PS.AGEX_IR & 0x07FF;
				if (temp2 & 0x0400) {
					temp2 |= 0xF800;
				}
				break;
		...
		temp2 = ((PS.AGEX_IR & 0x00FF) << 1);

		if (Get_ADDRESSMUX(PS.AGEX_CS)) {
			NEW_PS.MEM_ADDRESS = temp1 & 0xFFFF;
		} else {
			NEW_PS.MEM_ADDRESS = temp2 & 0xFFFF;
		}

		NEW_PS.MEM_NPC = PS.AGEX_NPC;
		NEW_PS.MEM_CC = PS.AGEX_CC;

		//MEM.ALU.RESULT logic
		int shf_type;
		switch (Get_ALU_RESULTMUX(PS.AGEX_CS)) {
			case 0:
				temp1 = PS.AGEX_IR & 0x000F;
				shf_type = ((PS.AGEX_IR & 0x0030) >> 4);
				switch (shf_type) {
					case 0x0:
						temp2 = PS.AGEX_SR1 << temp1;
						break;
					case 0x1:
						temp2 = PS.AGEX_SR1 >> temp1;
						break;
				...
				NEW_PS.MEM_ALU_RESULT = temp2 & 0xFFFF;
				break;
			case 1:
				if (Get_SR2MUX(PS.AGEX_CS)) {
					temp2 = PS.AGEX_IR & 0x001F;
					if (temp2 & 0x0010) {
						temp2 |= 0xFFE0;
					}
				} else {
				...
				switch (Get_ALUK(PS.AGEX_CS)) {
					case 0:
						NEW_PS.MEM_ALU_RESULT = (temp1 + temp2) & 0xFFFF;
						break;
					case 1:
						NEW_PS.MEM_ALU_RESULT = (temp1 & temp2) & 0xFFFF;
						break;
...
void DE_stage() {
	...
	//br.stall logic
	v_de_br_stall = PS.DE_V && CONTROL_STORE[CONTROL_STORE_ADDRESS][15]; // DE.V && BR.STALL

	//DEP.STALL logic
	dep_stall = 0;
	if (PS.DE_V) {
		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][0]) { //sr1.needed
			if (!dep_stall && v_agex_ld_reg) {
				dep_stall = (SR1 == agex_drid);
			}

			if (!dep_stall && v_mem_ld_reg) {
				dep_stall = (SR1 == mem_drid);
			}

			if (!dep_stall && v_sr_ld_reg) {
				dep_stall = (SR1 == sr_drid);
			}
		}

		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][1]) { //sr2.needed
			if (!dep_stall && v_agex_ld_reg) {
				dep_stall = (SR2 == agex_drid);
			}

			if (!dep_stall && v_mem_ld_reg) {
				dep_stall = (SR2 == mem_drid);
			}

			if (!dep_stall && v_sr_ld_reg) {
				dep_stall = (SR2 == sr_drid);
			}
		}

		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][12]) { //br.op
			if (!dep_stall && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc)) {
				dep_stall = 1;
			}
		}
	}
	...
	int ii, jj = 0;
	int LD_AGEX = (!mem_stall); // i had the future br_stalls in here, but I think that's incorrect
	...
	if (LD_AGEX) {
		...
		if (CONTROL_STORE[CONTROL_STORE_ADDRESS][2]) { //drmux
...
void FETCH_stage() {
	...
	icache_access(PC, &data, &icache_r); // should this be the old PC or the new one???? It's currently the old PC

	int stalling = (dep_stall || v_de_br_stall || v_agex_br_stall || v_mem_br_stall || mem_stall);

	int LD_PC = !stalling && icache_r;
	int LD_DE = !dep_stall && !mem_stall;
	
	if (LD_PC || (mem_pcmux)) { // if mem_pcmux is target_pc or trap_pc, then override the stalls
		switch (mem_pcmux) {
			case 0:
				NEW_PC = OLD_PC + 2;
				break;
		...
		NEW_PC = OLD_PC; // have to set NEW_PC to a value b/c PC is set to NEW_PC at the end of the function
	}

	PC = NEW_PC & 0xFFFF;

	if (LD_DE) {
		NEW_PS.DE_V = icache_r && !v_de_br_stall && !v_agex_br_stall && ! v_mem_br_stall; // DE.V = ICACHE.R
		NEW_PS.DE_NPC = OLD_PC + 2;
		NEW_PS.DE_IR = data & 0xFFFF; // pulled from icache
	}
	 ...
	 *
	 *
	 * if (!stalls && ICACHE.R) switch case with MEM.PCMUX which comes from the MEM stage
	 *
	 * access the icache
	 *
	 * if (!stalls) {
